...
const ProductCard = ({
    ...
    if (orientation === "horizontal") {
        ...
        return (
            // Compact vertical card, responsive
            <div className="w-full max-w-[260px] bg-slate-100/40 backdrop-blur-lg shadow-sm flex items-center justify-center flex-col">
                <Link to={`${routes.products.path}/${product.slug}`}>
                ...
                <div className="w-full h-full flex flex-col justify-between gap-1.5 py-2.5 px-3 border-b border-l border-r rounded-b-lg border-neutral-300">
                    <div>
                        {productCategory && (
                            <Badge size="sm">{productCategory.name}</Badge>
                        )}
